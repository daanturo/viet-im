...
def truthy(arg) -> bool:
    """If ARG is truthy, return True, else False.  Consider values other than
    False and None truthy, such as 0, [], empty string (\"\"), etc."""
    return (
        (not ((False == arg) and isinstance(arg, bool)))  # python: (0 == False) : True
        and (not (arg is None))  # None
    )
...
def _get_words_and_rhymes_fresh():
...
_cached_words_and_rhymes = None


def get_words_and_rhymes():
    global _cached_words_and_rhymes
    if _cached_words_and_rhymes is None:
        _cached_words_and_rhymes = _get_words_and_rhymes_fresh()
    return _cached_words_and_rhymes
...
def get_full_rhyme_table():
    ...
    if _cached_full_rhyme_table is None:
        ...
        with open("generated-rhyme-table.json", "w") as f:
            json.dump(_cached_full_rhyme_table, f, ensure_ascii=False, indent=2)
    ...
    for diacritic_name in diacritic_table.keys():
        ...
        for trigger in trigger_chars:
            ...
            match None:
                ...
                # Doesn't change, but same tone typed twice -> remove tone
                case _ if diacritic_name == rhyme_obj._marked_tone_name:
                    rules[key] = pre + rhyme_obj.without_tone() + trigger
                # Doesn't change, but same diacritic typed twice -> remove diacritic
...
def _diacritic_changes_when_insert():
    ...
    def helper(short, long, tone_str):
        inserting_char = long[len(short)][0]
        base_inserting_char = remove_string_all_diacritics(inserting_char)
        ...
        # "ứo"->"ướ".
        if not (
            prelim_table.get(new_no_tone)
            or any(filter(lambda s: s.startswith(new_no_tone), rhymes))
        ):
            new_no_tone = remove_string_tone_diacritics(short) + inserting_char
        k = unicode_norm(
            "NFC",
            short[:tone_pos_in_short_1]
            + tone_str
            + short[tone_pos_in_short_1:]
            + remove_string_all_diacritics(inserting_char),
        )
        # no "úa" -> "uá"
        ...
        v = unicode_norm(
            "NFC",
            new_no_tone[:tone_pos_in_long_1]
            + tone_str
            + new_no_tone[tone_pos_in_long_1:],
        )
        return (k, v)
...
def _compose_diacritic_at_end(candidate: str, im_name: str):
    ...
    # Fixed right boundary, let left boundary runs from left most to right most
...
def _syllable_candidate_regex(im_name):
    d_trigger = LEAD_DIACRITICS["d"][im_name]
    triggers = (
        "".join(
            trigger
            for v in (ALPHABET_DIACRITIC_TABLE | TONE_TABLE).values()
            for trigger in v[im_name]
        )
        + d_trigger
    )
    # fjwz: a bit of non-standard, doesn't affect software handling really
    re1 = f"(d{d_trigger}|qu|gi|{'|'.join(get_words_and_rhymes()['prefix_consonants'])}|[fjwz])?"
    re2 = f"([{VOWEL_LETTERS}]|[{triggers}])*"
    re3 = f"({'|'.join(get_words_and_rhymes()['suffix_consonants'])})?[{triggers}]*"
    composable_re = rf"{re1}{re2}{re3}"
    return composable_re


def compose_diacritics(candidate: str, im_name: str, strict=True):
    im_name_1 = "im_" + im_name.removeprefix("im_")
    if "" == candidate:
        return ""
    if strict and (not re.match(f"^{_syllable_candidate_regex(im_name_1)}$", candidate)):
        return candidate
    # Fixed left boundary, let right boundary runs from left most to right most
    p1 = candidate[0]
    for i in range(1, len(candidate)):
        s = p1 + candidate[i]
        composed_s = _compose_diacritic_at_end(s, im_name_1)
        p1 = composed_s
    return p1
